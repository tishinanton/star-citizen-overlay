...
Rockfall's local thumbnail and interactive-preview path is deliberately bounded:
...
- Rockfall validates the GLB envelope, embedded buffer accessors, structural
  complexity, 64 MiB byte limit, and 250,000-triangle limit before use;
- the same archive-fingerprinted GLB feeds both the deterministic 256×256
  software thumbnail and the sandboxed interactive Three.js viewer, so opening
  the viewer does not reconvert geometry; and
- the validated untextured GLB and PNG are retained under Electron
  `userData/blueprint-thumbnails`, keyed by archive size/mtime, renderer schema,
  output class, and asset path.
...
browser keeps its static icon, software thumbnail, or generic glyph. `.skin`,
`.skinm`, `.chr`, skeletal animation, external model resources, and
material/texture rendering are explicitly unsupported.
...
source geometry is deleted after every request; only the validated untextured
GLB and its PNG derivative remain in the local archive-fingerprinted Electron
cache. The renderer receives bounded bytes over a blueprint-ID IPC method, not
a path or file URL. Rockfall never uploads, publishes, commits, or transmits
extracted assets or generated previews.
